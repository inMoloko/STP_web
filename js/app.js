...
angular.module('myApp', ['nvd3'])
    .controller('myCtrl', function($scope){
        ...
        $scope.setDiapasone = function (d_prm) {
            var cur = $scope.data[0].values[$scope.data[0].values.length - 1].x;
            ...
            $scope.options.chart.brushExtent = [now, cur];
        };
        $scope.OpenReportPage = function (pageName) {
            var curD = $scope.options.chart.brushExtent;
            var left, right;
            if (curD == undefined)
            {
        ...
        var left = getUrlParam('left', window.location.href);
        var right = getUrlParam('right', window.location.href);

        if (left != null && right != null && left != undefined && right != undefined)
        {
            $scope.options.chart.brushExtent = [left, right];
        }
